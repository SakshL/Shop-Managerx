...
module.exports = async (client) => {
    ...
    client.on("guildMemberAdd", member => {
        ...
        if (member.user.bot) {
            setTimeout(async () => {
                member.roles.set([`${mainconfig.MemberID}`]).catch(() => {})
            }, 5000)
            return;
        }
        if (!member.user.bot) {
...
**Other than that! ENJOY!** :v:`)]}).catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)});
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (mem.roles.cache.has(``${mainconfig.MemberID}``)) mem.roles.remove(``${mainconfig.MemberID}`).catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)});
